...
static unsigned long long genHash(const char* fileName) {
    ...
    if (length > 0) {
        hash = *(extU - 1) | ((length > 2 ? *(ext - 2) : 0) << 8) | (length << 16) | (fileNameLowerU[0] << 24);
    }
    ...
    if (strlen(ext) > 0) {
        ...
        unsigned long long temp = static_cast<unsigned long long>(genHashInt(fileNameLowerU + 1, extU - 2));
...
DummyBSA::DummyBSA(unsigned long bsaVersion)
    : m_Version(bsaVersion), m_FolderName(""), m_FileName("dummy.dds"), m_TotalFileNameLength(0) {}
