#include "game_gamebryo/gamebryobsainvalidation.h"
#include "game_gamebryo/dummybsa.h"
...
void GamebryoBSAInvalidation::deactivate(MOBase::IProfile* profile) {
    ...
    if (!::WritePrivateProfileStringW(L"Archive", L"bInvalidateOlderFiles", L"0", iniFile.toStdWString().c_str()) ||
        !::WritePrivateProfileStringW(L"Archive", L"SInvalidationFile", L"ArchiveInvalidation.txt",
                                      iniFile.toStdWString().c_str())) {
        throw MOBase::MyException(QObject::tr("failed to deactivate BSA invalidation in \"%1\" (errorcode %2)")
                                      .arg(iniFile, ::GetLastError()));
    }
}
...
void GamebryoBSAInvalidation::activate(MOBase::IProfile* profile) {
    ...
    if (!::WritePrivateProfileStringW(L"Archive", L"bInvalidateOlderFiles", L"1", iniFile.toStdWString().c_str()) ||
        ...
        throw MOBase::MyException(
            QObject::tr("failed to activate BSA invalidation in \"%1\" (errorcode %2)").arg(iniFile, ::GetLastError()));
    }
}
