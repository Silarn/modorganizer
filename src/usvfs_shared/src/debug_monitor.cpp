...
void DebugMonitor::setProcessFilter(DWORD processId) { m_ProcessFilter = processId; }
...
void DebugMonitor::processDebugEvent() {
    ...
    if (ret == WAIT_OBJECT_0) {
        if ((m_DebugBuffer->processId == NO_FILTER) || (m_DebugBuffer->processId == m_ProcessFilter)) {
...
bool DebugMonitor::openSHM(LPCTSTR name) {
    ...
    if (m_BufferMapping == nullptr) {
        m_BufferMapping =
            ::CreateFileMapping(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sizeof(DebugBuffer), name);
