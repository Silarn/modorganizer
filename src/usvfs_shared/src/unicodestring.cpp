...
UnicodeString::UnicodeString(HANDLE fileHandle) { setFromHandle(fileHandle); }
...
size_t UnicodeString::size() const { return m_Buffer.size() > 0 ? m_Buffer.size() - 1 : 0; }

void UnicodeString::resize(size_t minSize) { m_Buffer.resize(minSize); }
...
UnicodeString& UnicodeString::appendPath(PUNICODE_STRING path) {
    if (path != nullptr) {
        ...
        m_Buffer.insert(m_Buffer.end(), path->Buffer, path->Buffer + (path->Length / sizeof(WCHAR)));
        update();
    }
    return *this;
}
...
void UnicodeString::setFromHandle(HANDLE fileHandle) {
    ...
    */
    ...
    SetLastError(0UL);
    DWORD res =
        GetFinalPathNameByHandleW(fileHandle, &m_Buffer[0], static_cast<DWORD>(m_Buffer.size()), FILE_NAME_NORMALIZED);
    if (res == 0) {
        m_Buffer.resize(0);
...
UnicodeString::operator LPCWSTR() const { return m_Buffer.data(); }
