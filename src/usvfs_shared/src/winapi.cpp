...
std::wstring getKnownFolderPath(REFKNOWNFOLDERID folderID) {
    ...
    ON_BLOCK_EXIT([writablePath]() { ::CoTaskMemFree(writablePath); });
...
OSVersion getOSVersion() {
    ...
    result.servicpack = versionInfo.wServicePackMajor << 16 | versionInfo.wServicePackMinor;
    return result;
}
...
std::string errorString(DWORD errorCode) {
    ...
    errorCode = errorCode != std::numeric_limits<DWORD>::max() ? errorCode : currentErrorCode;
...
std::string toString(const FILETIME& time) {
    ...
    stream << temp.wYear << "-" << temp.wMonth << "-" << temp.wDay << temp.wHour << ":" << temp.wMinute << ":"
           << temp.wSecond;
    return stream.str();
}
...
std::vector<FileResult> quickFindFiles(LPCWSTR directoryName, LPCWSTR pattern) {
    ...
    HANDLE hdl = CreateFileW(directoryName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                             nullptr, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, nullptr);

    ON_BLOCK_EXIT([hdl]() { CloseHandle(hdl); });
    ...
    while (res == STATUS_SUCCESS) {
        ...
        res = NtQueryDirectoryFile(hdl, nullptr, nullptr, nullptr, &status, buffer, BUFFER_SIZE,
        ...
        if (res == STATUS_SUCCESS) {
            ...
            while (info < endPos) {
                ...
                if (info->NextEntryOffset == 0) {
                    ...
                    info = reinterpret_cast<FILE_FULL_DIR_INFORMATION*>(reinterpret_cast<uint8_t*>(info) +
...
void createPath(LPCWSTR path, LPSECURITY_ATTRIBUTES securityAttributes) {
    std::unique_ptr<wchar_t, decltype(std::free)*> pathCopy{_wcsdup(path), std::free};

    // writable copy of the path
    wchar_t* current = pathCopy.get();

    if ((wcsncmp(current, LR"(\\?\)", 4) == 0) || (wcsncmp(current, LR"(\??\)", 4) == 0)) {
        current += 4;
    }
            ...
            if (!::CreateDirectoryW(pathCopy.get(), securityAttributes)) {
                ...
                if ((err != ERROR_ALREADY_EXISTS) && (err != NOERROR)) {
                    throw usvfs::shared::windows_error(ush::string_cast<std::string>(
                        fmt::format(L"failed to create intermediate directory {}", pathCopy.get())));
                }
                // restore the path
            }
