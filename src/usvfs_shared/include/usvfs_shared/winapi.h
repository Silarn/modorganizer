...
struct Result {
    ...
    Result(Result&& reference)
        : valid(reference.valid), startupInfo(reference.startupInfo), processInfo(reference.processInfo),
    ...
    size_t readStdout(std::vector<uint8_t>& buffer, bool& eof) {
        if (stdoutPipe != INVALID_HANDLE_VALUE) {
            DWORD read;
            BOOL res = ReadFile(stdoutPipe, &buffer[0], static_cast<DWORD>(buffer.size()), &read, nullptr);
            eof = (res == TRUE) && (read == 0);
  ...
  public:
    ...
    _Create(_Create<CharT>&& reference)
        : m_CurrentDirectory(std::move(reference.m_CurrentDirectory)),
          m_ProcessAttributes(reference.m_ProcessAttributes), m_ThreadAttributes(reference.m_ThreadAttributes),
          m_InheritHandles(reference.m_InheritHandles), m_CreationFlags(std::move(reference.m_CreationFlags)),
    ...
    Result operator()() {
        ...
        result.valid = createProcessInt(nullptr, clBuffer.get(), m_ProcessAttributes, m_ThreadAttributes,
                                        m_InheritHandles, m_CreationFlags, nullptr,
...
class _Create {
  public:
    ...
    HANDLE operator()() { return callDelegate(std::integral_constant<bool, sizeof(CharT) == sizeof(wchar_t)>()); }
 ...
 * @param isDirectory (optional) if this isn't null, it will be set to true if the path specifies a directory, false
 * otherwise
 ...
 */
void createPath(LPCWSTR path, LPSECURITY_ATTRIBUTES securityAttributes = nullptr);
} // namespace wide
} // namespace ex
