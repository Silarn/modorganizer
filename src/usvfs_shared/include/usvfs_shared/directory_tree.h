...
namespace usvfs {
namespace shared {
template <typename T, typename U> struct SHMDataCreator {
 ...
 */
template <typename NodeDataT> class DirectoryTree {
  template <typename T> friend class TreeContainer;
...
public:
   ...
   **/
  DirectoryTree(const std::string &name, TreeFlags flags,
                const NodePtrT &parent, const NodeDataT &data,
  ...
  void findLocal(std::vector<NodePtrT> &output,
    ...
    for (auto iter = m_Nodes.begin(); iter != m_Nodes.end(); ++iter) {
      ...
      if (pattern.size() > 1 && (pattern[0] == '*') &&
        ...
        iter->second->findLocal(output, pattern.substr(1));
                      ...
                      wildcard::PartialMatch(iter->second->name().c_str(),
 ...
 * reallocated
 */
template <typename TreeT> class TreeContainer {
...
private:
  ...
  template <typename T>
  TreeT *createSubNode(const VoidAllocatorT &allocator, const std::string &name,
  ...
  template <typename T>
  typename TreeT::NodePtrT addNode(TreeT *base, const fs::path &name,
                                   fs::path::iterator iter, const T &data,
    ...
    if (next == name.end()) {
      ...
      if (subNode == base->m_Nodes.end()) {
        typename TreeT::NodePtrT newNode = createSubPtr(
            createSubNode(allocator, iter->string(),
                          FLAG_DIRECTORY | FLAG_DUMMY, createEmpty()));
        subNode =
            base->m_Nodes.insert(std::make_pair(iterString, newNode)).first;
        subNode->second->m_Self = TreeT::WeakPtrT(subNode->second);
        subNode->second->m_Parent = base->m_Self;
      }
      return addNode(subNode->second.get().get(), name, next, data, overwrite,
                     flags, allocator);
    }
  }
  ...
  TreeMeta *createOrOpen(const char *SHMName, size_t size) {
    ...
    try {
      ...
      newSHM = new SharedMemoryT(bi::create_only, SHMName,
  ...
  TreeMeta *activateSHM(SharedMemoryT *shm, const char *SHMName) {
    ...
    m_SHM.reset(shm);
    std::pair<TreeMeta *, SharedMemoryT::size_type> res =
        m_SHM->find<TreeMeta>("Meta");
    bool lastUser = false;
    if (res.first == nullptr) {
      res.first = m_SHM->construct<TreeMeta>("Meta")(
          createEmpty(), m_SHM->get_segment_manager());
      if (res.first == nullptr) {
  ...
  std::string followupName() const {
    ...
    if (match.size() != 3) {
      USVFS_THROW_EXCEPTION(usage_error()
                            << ex_msg("shared memory name invalid"));
    }
  ...
  void reassign() const {
    ...
    for (;;) {
      ...
      self->m_TreeMeta =
          self->createOrOpen(nextName.c_str(), m_SHM->get_size() * 2);

      if (!m_TreeMeta->outdated) {
        break;
      }
    }
    spdlog::get("usvfs")->info("tree {0} size now {1} bytes", m_SHMName,
                               m_SHM->get_size());
  }
