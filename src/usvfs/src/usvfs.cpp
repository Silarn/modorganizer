...
#pragma message("need a customized name for the shm")
    ...
    if (logger.get() == nullptr) {
      logger = toConsole
                   ? spdlog::create<spdlog::sinks::stdout_sink_mt>("hooks")
...
void createMiniDump(PEXCEPTION_POINTERS exceptionPtrs) {
  ...
  if (dbgDLL) {
    ...
    if (funcDump) {
      ...
      std::wstring dmpPath =
          winapi::wide::getKnownFolderPath(FOLDERID_LocalAppData) +
          L"\\usvfs\\uvsfs_x64.dmp";
#else
      std::wstring dmpPath =
          winapi::wide::getKnownFolderPath(FOLDERID_LocalAppData) +
          L"\\usvfs\\uvsfs_x86.dmp";
#endif
      std::wstring parent = bfs::path(dmpPath).parent_path().wstring();
      winapi::ex::wide::createPath(parent.c_str());
      HANDLE dumpFile = winapi::wide::createFile(dmpPath)
                            .createAlways()
      ...
      if (dumpFile != INVALID_HANDLE_VALUE) {
        ...
        if (success) {
          ...
          if (logger != nullptr) {
            logger->error("No crash dump created, errorcode: {}",
                          GetLastError());
          }
        }
      } else {
        if (logger != nullptr) {
          logger->error(
              "No crash dump created, failed to open \"{}\" for writing",
              ush::string_cast<std::string>(dmpPath));
        }
      }
    } else {
...
LONG WINAPI VEHandler(PEXCEPTION_POINTERS exceptionPtrs) {
  if ((exceptionPtrs->ExceptionRecord->ExceptionCode <
       0x80000000) // non-critical
  ...
  */
  ...
  if (RemoveVectoredExceptionHandler(exceptionHandler) == 0) {
    ::MessageBoxA(
        nullptr,
        "Failed to properly report windows exception, not daring to continue",
  ...
  try {
    std::pair<uintptr_t, uintptr_t> range =
        winapi::ex::getSectionRange(dllModule);

    uintptr_t exceptionAddress = reinterpret_cast<uintptr_t>(
    ...
    if ((exceptionAddress < range.first) || (exceptionAddress > range.second)) {
      ...
      std::wstring modName = winapi::ex::wide::getSectionName(
          exceptionPtrs->ExceptionRecord->ExceptionAddress);
      if (logger.get() != nullptr) {
...
#pragma message("bug: if the ve handler is called, the process breaks")
  ...
  spdlog::get("usvfs")->debug(
      "inithooks called {0} in process {1} (log level {2})",
      params->instanceName, ::GetCurrentProcessId(),
      static_cast<int>(params->logLevel));
  spdlog::get("usvfs")->info("process name: {}",
...
void WINAPI USVFSInitParameters(USVFSParameters *parameters, const char *instanceName, bool debugMode, LogLevel logLevel) {
