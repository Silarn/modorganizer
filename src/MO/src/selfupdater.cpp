...
SelfUpdater::SelfUpdater(NexusInterface* nexusInterface)
    : m_Parent(nullptr), m_Interface(nexusInterface), m_UpdateRequestID(-1), m_Reply(nullptr), m_Attempts(3),
...
void SelfUpdater::testForUpdate() {
    ...
    if (m_UpdateRequestID == -1 && m_NexusDownload != nullptr) {
        m_UpdateRequestID = m_Interface->requestDescription(m_NexusDownload->nexusModOrganizerID(), this, QVariant(),
...
void SelfUpdater::startUpdate() {
    ...
    if ((m_UpdateRequestID == -1) && (!m_NewestVersion.isEmpty())) {
        if (QMessageBox::question(
                m_Parent, tr("Update"),
                tr("An update is available (newest version: %1), do you want to install it?").arg(m_NewestVersion),
                QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes) {
            m_UpdateRequestID = m_Interface->requestFiles(m_NexusDownload->nexusModOrganizerID(), this, m_NewestVersion,
                                                          "", m_NexusDownload);
        }
    }
}
...
void SelfUpdater::download(const QString& downloadLink, const QString& fileName) {
    ...
    m_UpdateFile.setFileName(
        QDir::fromNativeSeparators(qApp->property("dataPath").toString()).append("/").append(fileName));
...
void SelfUpdater::installUpdate() {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        if (file.exists() && file.isFile()) {
            if (!shellMove(QStringList(mopath + "/" + outputName), QStringList(backupPath + "/" + outputName))) {
                reportError(tr("failed to move outdated files: %1. Please update manually.")
...
void SelfUpdater::nxmFilesAvailable(int, QVariant userData, QVariant resultData, int requestID) {
            ...
            if (fileInfo["category_id"].toInt() == 2) {
                ...
                if (updateExpList.indexIn(description) != -1) {
                    ...
                    QStringList supportedVersions =
                        updateExpList.cap(1).split(QRegExp(",[ ]*"), QString::SkipEmptyParts);
                    ...
                    if ((rangeLowEnd <= m_MOVersion) && (m_MOVersion <= rangeHighEnd)) {
                        updateFileID = fileInfo["id"].toInt();
                        updateFileName = fileInfo["uri"].toString();
                        break;
                    } else {
                    ...
                    qWarning("invalid update description: %s", description.toUtf8().constData());
                }
    ...
    if (updateFileID != -1) {
        ...
        m_UpdateRequestID = m_Interface->requestDownloadURL(m_NexusDownload->nexusModOrganizerID(), updateFileID, this,
                                                            updateFileName, "", m_NexusDownload);
        ...
        if (QMessageBox::question(m_Parent, tr("Update"),
                                  tr("No incremental update available for this version, "
                                     "the complete package needs to be downloaded (%1 kB)")
                                      .arg(mainFileSize),
                                  QMessageBox::Ok | QMessageBox::Cancel) == QMessageBox::Ok) {
            m_UpdateRequestID = m_Interface->requestDownloadURL(m_NexusDownload->nexusModOrganizerID(), mainFileID,
                                                                this, mainFileName, "", m_NexusDownload);
        }
    } else {
...
void SelfUpdater::setNexusDownload(MOBase::IPluginGame const* game) { m_NexusDownload = game; }
