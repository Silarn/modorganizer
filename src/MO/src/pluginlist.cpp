...
QString PluginList::getColumnToolTip(int column) {
    ...
    case COL_MODINDEX:
        return tr("The modindex determins the formids of objects originating from this mods.");
    default:
        return tr("unknown");
    }
}

void PluginList::refresh(const QString& profileName, const DirectoryEntry& baseDirectory, const QString& pluginsFile,
                         const QString& loadOrderFile, const QString& lockedOrderFile) {
...
bool PluginList::readLoadOrder(const QString& fileName) {
    ...
    while (!file.atEnd()) {
        ...
        if ((modName.size() > 0) && (m_ESPLoadOrder.find(modName) == m_ESPLoadOrder.end()) &&
...
void PluginList::saveTo(const QString& pluginFileName, const QString& loadOrderFileName,
                        const QString& lockedOrderFileName, const QString& deleterFileName, bool hideUnchecked) const {
...
QStringList PluginList::masters(const QString& name) const {
    ...
    if (iter == m_ESPsByName.end()) {
        ...
        QStringList result;
        foreach (const QString& master, m_ESPs[iter->second].m_Masters) { result.append(master); }
        return result;
    }
}
    ...
    if ((role == Qt::DisplayRole) || (role == Qt::EditRole)) {
        ...
        if (m_ESPs[index].m_ForceEnabled) {
            ...
            if (m_ESPs[index].m_HasIni) {
                ...
                text += "<br>This file is a dummy! It exists only so the bsa with the same name gets loaded. If you "
                        "let MO manage archives you "
                        "don't need this: Enable the archive with the same name in the \"Archive\" tab and disable "
                        "this plugin.";
            }
            toolTip += text;
        }
...
void PluginList::setPluginPriority(int row, int& newPriority) {
    ...
    if (newPriorityTemp < 0)
        ...
        newPriorityTemp = m_ESPsByPriority.size() - 1;
...
void PluginList::managedGameChanged(IPluginGame const* gamePlugin) { m_GamePlugin = gamePlugin; }
