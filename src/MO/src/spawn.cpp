...
static bool spawn(LPCWSTR binary, LPCWSTR arguments, LPCWSTR currentDirectory, bool suspended, HANDLE stdOut,
    ...
    BOOL success =
        ::CreateProcess(nullptr, commandLine, nullptr, nullptr, // no special process or thread attributes
...
HANDLE startBinary(const QFileInfo& binary, const QString& arguments, const QString& profileName, int logLevel,
                   const QDir& currentDirectory, bool hooked, HANDLE stdOut, HANDLE stdErr) {
    JOBOBJECT_EXTENDED_LIMIT_INFORMATION jobInfo;

    ::QueryInformationJobObject(nullptr, JobObjectExtendedLimitInformation, &jobInfo,
                                sizeof(JOBOBJECT_EXTENDED_LIMIT_INFORMATION), nullptr);
    ...
    if (jobObject == nullptr) {
        ...
        ::SetInformationJobObject(jobObject, JobObjectExtendedLimitInformation, &jobInfo,
    ...
    try {
        if (!spawn(binaryName.c_str(), ToWString(arguments).c_str(), currentDirectoryName.c_str(), true, stdOut, stdErr,
