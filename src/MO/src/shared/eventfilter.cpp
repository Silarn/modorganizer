...
void injectDLL(HANDLE processHandle, HANDLE threadHandle, const std::string& dllname, const std::wstring& profileName,
    ...
    _snprintf(parameters.initstr, 5,
              "Init"); // this is the name of thie initialisation function we want to call in the target process
    ...
    LPVOID remoteMem =
        ::VirtualAllocEx(processHandle, nullptr, sizeof(TParameters), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    ...
    // binary. This code will load our injected dll into the process and run its Init-function which takes the profile
    // name as its parameter obviously this code is ultra-hacky
    ...
    PBYTE stubRemote = reinterpret_cast<PBYTE>(
        ::VirtualAllocEx(processHandle, nullptr, sizeof(stubLocal), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
    ...
    // now replace each baadf00d by the correct value. The pointers need to be the pointers in the REMOTE memory of
    // course, that's why we copied them there
    ...
    *(PULONG)&stubLocal[42] = entryPoint;
    ...
    if (!::WriteProcessMemory(processHandle, stubRemote, reinterpret_cast<LPCVOID>(stubLocal), sizeof(stubLocal),
                              &written) ||
