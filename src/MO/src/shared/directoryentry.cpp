...
bool FileEntry::removeOrigin(int origin) {
    if (m_Origin == origin) {
        if (!m_Alternatives.empty()) {
            ...
            for (std::vector<int>::iterator iter = m_Alternatives.begin(); iter != m_Alternatives.end(); ++iter) {
                if ((m_Parent->getOriginByID(*iter).getPriority() >
                     m_Parent->getOriginByID(*currentIter).getPriority()) &&
                    (*iter != origin)) {
                    currentIter = iter;
                }
            }
...
static bool ByOriginPriority(DirectoryEntry* entry, int LHS, int RHS) {
    int l = entry->getOriginByID(LHS).getPriority();
    if (l < 0)
        l = INT_MAX;
    int r = entry->getOriginByID(RHS).getPriority();
    if (r < 0)
        r = INT_MAX;
