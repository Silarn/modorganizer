...
bool ModList::dropURLs(const QMimeData* mimeData, int row, const QModelIndex& parent) {
    ...
    QDir gameDirectory(qApp->property("dataPath").toString() + "/" +
...
void ModList::removeRowForce(int row, const QModelIndex& parent) {
    ...
    m_Profile->modlistWriter()
        .write(); // this ensures the modified list gets written back before new mods can be installed
