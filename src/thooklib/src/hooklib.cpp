...
void WriteSingleJump(THookInfo& hookInfo, HookError* error) {
    ...
    if (!VirtualProtect(hookInfo.originalFunction, JUMP_SIZE, PAGE_EXECUTE_WRITECOPY, &oldprotect)) {
...
BOOL HookChainHook(THookInfo& hookInfo, HookError* error) {
    return HookChainHook(hookInfo, reinterpret_cast<LPBYTE>(hookInfo.originalFunction), error);
}
...
BOOL HookDisasm(THookInfo& hookInfo, HookError* error) {
    ...
    while (size < minSize) {
        ...
        if ((ud_insn_mnemonic(disasm()) == UD_Iret) || (ud_insn_mnemonic(disasm()) == UD_Iint3)) {
            if (error != nullptr) {
                *error = ERR_FUNCEND;
            }
            return FALSE;
    ...
    if (hookInfo.stub) {
        hookInfo.trampoline = TrampolinePool::instance().storeStub(hookInfo.replacementFunction,
                                                                   hookInfo.originalFunction, size, &rerouteOffset);
    } else {
        hookInfo.trampoline = TrampolinePool::instance().storeTrampoline(
...
HOOKHANDLE applyHook(THookInfo info, HookError* error) {
    ...
    default: { success = HookDisasm(info, error); } break;
    }
...
void HookLib::RemoveHook(HOOKHANDLE handle) {
    ...
    if (iter != s_Hooks.end()) {
        ...
        if (info.type == THookInfo::TYPE_HOTPATCH) {
            ...
            *(address + 1) = 0xff;
        ...
        } else if ((info.type == THookInfo::TYPE_OVERWRITE) || (info.type == THookInfo::TYPE_WIN64PATCH)) {
            DWORD oldProtect = 0;
            ...
            DWORD oldProtect = 0;
            if (!VirtualProtect(jumpPos, info.preamble.size(), PAGE_EXECUTE_WRITECOPY, &oldProtect)) {
                throw shared::windows_error("failed to gain write access to remove hook");
            }
