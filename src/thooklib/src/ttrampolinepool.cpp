...
LPVOID TrampolinePool::storeStub(LPVOID reroute, LPVOID original, LPVOID returnAddress) {
    ...
    *reinterpret_cast<LPVOID*>(spot) = reroute;
    ...
    size_t codeSize = rcode.getCodeSize();

    m_MaxTrampolineSize = std::max(m_MaxTrampolineSize, static_cast<int>(codeSize + sizeof(LPVOID)));
...
#if IS_X64
    // insert backup code
    *rerouteOffset = rcode.getCodeSize();
...
LPVOID TrampolinePool::storeTrampoline(LPVOID reroute, LPVOID original, size_t preambleSize, size_t* rerouteOffset) {
    ...
    *rerouteOffset = rcode.getCodeSize();
    ...
    // copy code to buffer
    codeSize = static_cast<size_t>(rcode.relocate(spot));
...
TrampolinePool::BufferMap::iterator TrampolinePool::allocateBuffer(LPVOID addressNear) {
    ...
    uintptr_t start = std::max(lowerEnd, reinterpret_cast<uintptr_t>(sysInfo.lpMinimumApplicationAddress));
    uintptr_t upperEnd = reinterpret_cast<uintptr_t>(rounded) + m_SearchRange;
    uintptr_t end = std::min(upperEnd, reinterpret_cast<uintptr_t>(sysInfo.lpMaximumApplicationAddress));

    LPVOID buffer = nullptr;
    for (uintptr_t cur = start; cur < end; cur += sysInfo.dwPageSize) {
        buffer =
            VirtualAlloc(reinterpret_cast<LPVOID>(cur), m_BufferSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        if (buffer != nullptr) {
            break;
        }
    }
    ...
    spdlog::get("usvfs")->debug("allocated trampoline buffer for jumps between {0:p} and {1:x} at {2:p}"
                                "(size {3})",
                                rounded, (reinterpret_cast<uintptr_t>(rounded) + m_SearchRange), buffer, m_BufferSize);
    return iter;
}
