...
void addStub(size_t userDataSize, X86Assembler& assembler, bool skipInit, TDataRemote* localData,
             TDataRemote* remoteData, LPCSTR initFunction) {
...
REGWORD WriteInjectionStub(HANDLE processHandle, LPCWSTR dllName, LPCSTR initFunction, LPCVOID userData,
    ...
    // now for the interesting part: write a stub into the target process that is run before any code of the original
    // binary.
    ...
    PBYTE stubRemote = reinterpret_cast<PBYTE>(
        VirtualAllocEx(processHandle, nullptr, stubSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
...
void InjectDLLEIP(HANDLE processHandle, HANDLE threadHandle, LPCWSTR dllName, LPCSTR initFunction, LPCVOID userData,
                  size_t userDataSize, bool skipInit) {
    threadHandle =
    ...
    REGWORD stubAddress =
        WriteInjectionStub(processHandle, dllName, initFunction, userData, userDataSize, skipInit, returnAddress);
...
void InjectDLLRemoteThread(HANDLE processHandle, LPCWSTR dllName, LPCSTR initFunction, LPCVOID userData,
                           size_t userDataSize, bool skipInit) {
    REGWORD stubAddress = WriteInjectionStub(processHandle, dllName, initFunction, userData, userDataSize, skipInit, 0);

    DWORD threadId = 0UL;

    // MessageBoxA(nullptr, fmt::format("address: {0:x} - {1:x}", stubAddress, GetProcessId(processHandle)).c_str(), "",
    // MB_OK);

    HANDLE threadHandle = CreateRemoteThread(
        processHandle, nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(stubAddress), nullptr, 0, &threadId);
    if (threadHandle == nullptr) {
...
void InjectLib::InjectDLL(HANDLE processHandle, HANDLE threadHandle, LPCWSTR dllName, LPCSTR initFunction,
                          LPCVOID userData, size_t userDataSize, bool skipInit) {
    namespace fs = std::experimental::filesystem;
    if (!fs::exists(fs::path(dllName))) {
