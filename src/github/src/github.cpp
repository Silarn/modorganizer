...
QJsonArray GitHub::releases(const Repository& repo) {
    QJsonDocument result =
        request(Method::GET, QString("repos/%1/%2/releases").arg(repo.owner, repo.project), QByteArray(), true);
    if (!result.isArray()) {
...
void GitHub::releases(const Repository& repo, const std::function<void(const QJsonArray&)>& callback) {
    request(Method::GET, QString("repos/%1/%2/releases").arg(repo.owner, repo.project), QByteArray(),
...
QJsonDocument GitHub::handleReply(QNetworkReply* reply) {
    ...
    if (statusCode != 200) {
        return QJsonDocument(
            QJsonObject({{"http_status", statusCode},
                         {"redirection", reply->attribute(QNetworkRequest::RedirectionTargetAttribute).toString()},
...
QNetworkReply* GitHub::genReply(Method method, const QString& path, const QByteArray& data, bool relative) {
...
QJsonDocument GitHub::request(Method method, const QString& path, const QByteArray& data, bool relative) {
    QEventLoop wait;
...
void GitHub::request(Method method, const QString& path, const QByteArray& data,
                     const std::function<void(const QJsonDocument&)>& callback, bool relative) {
    ...
    connect(reply, &QNetworkReply::finished, [this, reply, timer, method, data, callback]() {
        ...
        timer->stop();
        if (object.value("http_status").toDouble() == 301.0) {
            request(method, object.value("redirection").toString(), data, callback, false);
        } else {
            callback(result);
        }
        reply->deleteLater();
    ...
    connect(reply, static_cast<void (QNetworkReply::*)(QNetworkReply::NetworkError)>(&QNetworkReply::error),
            [reply, timer, callback](QNetworkReply::NetworkError error) {
                ...
                timer->stop();
                callback(QJsonDocument(QJsonObject({{"network_error", reply->errorString()}})));
                reply->deleteLater();
            });
